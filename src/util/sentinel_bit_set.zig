...
pub fn SentinelArrayBitSet(comptime MaskIntType: type, comptime size: usize, comptime sentinel: MaskIntType) type {
    ...
    return extern struct {
        ...
        masks: [num_masks:sentinel]MaskInt,

        /// Creates a bit set with no elements present.
        pub fn initEmpty() Self {
            return .{ .masks = @bitCast([_]MaskInt{0} ** num_masks ++ [0:sentinel]MaskInt{}) };
        }
        ...
        pub fn initFull() Self {
            if (num_masks == 0) {
                ...
                return .{ .masks = @bitCast([_]MaskInt{~@as(MaskInt, 0)} ** (num_masks - 1) ++ [_]MaskInt{last_item_mask} ++ [0:sentinel]MaskInt{}) };
            }
        }
